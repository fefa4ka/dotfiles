...
" Function: #get_diff_arc {{{1
function! sy#repo#get_diff_arc(sy, exitval, diff) abort
  call sy#verbose('get_diff_arc()', 'arc')
  let [found_diff, diff] = a:exitval ? [0, []] : [1, a:diff]
  call s:get_diff_end(a:sy, found_diff, 'arc', diff)
endfunction
...
let s:default_vcs_cmds_diffmode = {
      ...
      \ 'hg':       'hg cat %f',
      \ 'svn':      'svn cat %f',
      \ 'arc':      'arc diff --git %f | patch --output=- --reverse',
      \ 'bzr':      'bzr cat %f',
